...
import {makeTelegramApi} from './util/makeTelegramApi.js'
// import { ensureNewlineAtEnd } from './util/ensure-new-line-at-end.js'
...
/**
  ...
  */
const replaceBlockquotesWithTelegramStyle = ({ text, html }) => {
  ...
  lines.forEach(line => {
      ...
      if (line.startsWith('>')) {
          // Если это начало цитаты, открываем <blockquote>
          if (!inQuote) {
              result += '<blockquote class="blockquote" data-entity-type="MessageEntityBlockquote">\n';
              inQuote = true;
          }
          // Убираем ">" и добавляем строку внутрь <blockquote>
          result += line.slice(1).trim() + '\n';
      } else {
          // Если строка не начинается с ">" и мы внутри цитаты, закрываем <blockquote>
          if (inQuote) {
              result += '</blockquote>\n';
              inQuote = false;
          }
  ...
  // Если текст закончился, а цитата не закрыта, закрываем <blockquote>
  if (inQuote) {
      result += '</blockquote>\n';
  }

  return {
    text, // Возвращаем исходный текст без изменений
    html: result, // Возвращаем HTML с обработанными цитатами
  };
};
...
/**
  ...
  * @param {string} content.html
  */
const getTextAndHtmlForTelegram = ({ text, html }) => {
  ...
  const processedText = text.replace(
     ...
     */
    (match, label, url) => {
      // Если URL не начинается с http:// или https://, возвращаем только текст
...
export function toTelegram(tree, options) {
  ...
  // @ts-ignore // FIXME: Сасать омерика
  ...
  /**
   * NOTE: This is not appropriate finctionality
   * for Telegram messeges
   */
  ...
  const [returnValue] = [result]
    .map(makeTelegramApi)
    // .map(getTextAndHtmlForTelegram)
    // .map(replaceBlockquotesWithTelegramStyle);
